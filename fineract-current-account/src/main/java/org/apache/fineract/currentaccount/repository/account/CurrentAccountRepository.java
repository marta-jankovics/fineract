...
public interface CurrentAccountRepository extends JpaRepository<CurrentAccount, String> {
    ...
    @Query("SELECT new org.apache.fineract.currentaccount.data.account.CurrentAccountData(ca.id, ca.accountNumber, ca.externalId, ca.clientId, "
            + "c.displayName, c.office.id, ca.productId, cp.name, cp.shortName, cp.description, cp.accountingType, ca.status, ca.activatedOnDate, "
            + "ca.allowOverdraft, ca.overdraftLimit, ca.allowForceTransaction, ca.minimumRequiredBalance, ca.balanceCalculationType, "
            + "cp.currency.code, cp.currency.digitsAfterDecimal, cp.currency.inMultiplesOf, curr.name, curr.displaySymbol) "
            + "FROM CurrentAccount ca, CurrentProduct cp, Client c, ApplicationCurrency curr WHERE ca.productId = cp.id AND "
            + "curr.code = cp.currency.code AND ca.clientId = c.id AND ca.id = :id")
    ...
    @Query("SELECT new org.apache.fineract.currentaccount.data.account.CurrentAccountData(ca.id, ca.accountNumber, ca.externalId, ca.clientId, "
            ...
            + "cp.currency.code, cp.currency.digitsAfterDecimal, cp.currency.inMultiplesOf, curr.name, curr.displaySymbol) "
            + "FROM CurrentAccount ca, CurrentProduct cp, Client c, ApplicationCurrency curr WHERE ca.productId = cp.id AND "
            + "curr.code = cp.currency.code  AND ca.clientId = c.id AND ca.externalId = :externalId")
    ...
    @Query("SELECT new org.apache.fineract.currentaccount.data.account.CurrentAccountData(ca.id, ca.accountNumber, ca.externalId, ca.clientId, "
            ...
            + "cp.currency.code, cp.currency.digitsAfterDecimal, cp.currency.inMultiplesOf, curr.name, curr.displaySymbol) "
            + "FROM CurrentAccount ca, CurrentProduct cp, Client c, ApplicationCurrency curr WHERE ca.productId = cp.id AND "
            + "curr.code = cp.currency.code AND ca.clientId = c.id  AND ca.accountNumber = :accountNumber")
    ...
    @Query("SELECT new org.apache.fineract.currentaccount.data.account.CurrentAccountData(ca.id, ca.accountNumber, ca.externalId, ca.clientId, "
            ...
            + "cp.currency.code, cp.currency.digitsAfterDecimal, cp.currency.inMultiplesOf, curr.name, curr.displaySymbol) "
            + "FROM CurrentAccount ca, CurrentProduct cp, Client c, ApplicationCurrency curr WHERE ca.productId = cp.id AND "
            + "curr.code = cp.currency.code AND ca.clientId = c.id")
    ...
    @Query("SELECT new org.apache.fineract.currentaccount.data.account.CurrentAccountData(ca.id, ca.accountNumber, ca.externalId, ca.clientId, "
            ...
            + "ca.allowOverdraft, ca.overdraftLimit, ca.allowForceTransaction, ca.minimumRequiredBalance, ca.balanceCalculationType, "
            + "cp.currency.code, cp.currency.digitsAfterDecimal, cp.currency.inMultiplesOf, curr.name, curr.displaySymbol) "
            + "FROM CurrentAccount ca, CurrentProduct cp, ApplicationCurrency curr, Client c WHERE ca.productId = cp.id AND "
            + "curr.code = cp.currency.code AND ca.clientId = c.id AND c.id = :clientId")
