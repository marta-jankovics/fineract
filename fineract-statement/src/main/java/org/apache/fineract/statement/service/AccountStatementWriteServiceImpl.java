...
public class AccountStatementWriteServiceImpl implements AccountStatementWriteService {
    ...
    public void createAccountStatements(@NotNull String accountId, @NotNull String productId, @NotNull PortfolioProductType productType,
        ...
        if (!preStatementCreate(accountId)) {
            throw new PlatformApiDataValidationException("error.msg.account.statement.create",
                    "Statement can not be created on Account " + accountId, PARAM_STATEMENTS, accountId);
        }
    ...
    protected void createDefaultAccountStatements(@NotNull String accountId, @NotNull String productId,
        ...
        String defaultSequencePrefix = getDefaultSequencePrefix(accountId);
    ...
    public Map<String, Object> updateAccountStatements(@NotNull String accountId, @NotNull PortfolioProductType productType,
            @NotNull JsonCommand command) {
        if (!preStatementCreate(accountId)) {
            throw new PlatformApiDataValidationException("error.msg.account.statement.update",
                    "Statement can not be updated on Account " + accountId, PARAM_STATEMENTS, accountId);
        }
