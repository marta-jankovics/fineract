...
public class ProductStatementWriteServiceImpl implements ProductStatementWriteService {
    ...
    public Map<String, Object> updateProductStatements(@NotNull String productId, @NotNull PortfolioProductType productType,
        ...
        if (command.parameterExists(PARAM_STATEMENTS)) {
            ...
            if (statementArray != null) {
                ...
                changes = new HashMap<>();
                AccountStatementWriteService accountStatementService = null;
                for (JsonElement statementElement : statementArray) {
                    ...
                    if (statementData.isInherit()) {
                        if (accountStatementService == null) {
                            accountStatementService = serviceProvider.getAccountStatementWriteService(productType);
                        }
                ...
                for (ProductStatement statement : statementsByCode.values()) {
                    ...
                    if (statementRepository.hasAccountReference(statement.getId())) {
                        throw new PlatformApiDataValidationException("error.msg.product.statement.delete",
                                "Product statement can not be deleted because it is used on Account level " + statementCode,
                                PARAM_STATEMENTS, statementCode);
                    }
