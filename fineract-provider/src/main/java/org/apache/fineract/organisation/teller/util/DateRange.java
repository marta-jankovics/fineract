...
public class DateRange {
    ...
    public static DateRange fromString(final String dateToParse) {
        ...
        final String testee;
        if (dateToParse == null) {
            testee = DateUtils.format(DateUtils.getBusinessLocalDate());
        } else {
            testee = dateToParse;
        }
        ...
        try {
            dateRange.setStartDate(DateUtils.parseLocalDate(tokenizer.nextToken()));
        ...
        if (tokenizer.hasMoreTokens()) {
            try {
                dateRange.setEndDate(DateUtils.parseLocalDate(tokenizer.nextToken()));
            } catch (DateTimeParseException ex) {
                LOG.error("Problem occurred in DateRange function Could not parse the date recieved.", ex);
            }
        }
